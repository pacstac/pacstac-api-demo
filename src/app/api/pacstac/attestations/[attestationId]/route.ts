...
export async function GET(request: Request, ctx: RouteContext<{ attestationId: string }>) {
  ...
  try {
    ...
    const url = new URL(`${base}/v1/attestations/${encodeURIComponent(attestationId)}`);
    ...
    console.error("[pacstac-proxy] attestation fetch failed", message);
    return new Response(
      JSON.stringify({
        error: "PACSTAC_FETCH_FAILED",
        message,
        target: "https://pacstac.com/v1/attestations/:attestationId",
        details: {
